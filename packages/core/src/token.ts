import { parse, walk } from 'css-tree'

import { Path } from './path'
import type { ToCacheKey } from './types'
import { hash } from './utils'
import type { TokenValue } from './values'
import {
  ColorParser,
  CommaParser,
  PxParser,
  RawParser,
  RemParser,
  ValueParserChain,
} from './values'

const valueParserChain = new ValueParserChain([
  new ColorParser(),
  new PxParser(),
  new RemParser(),
  new CommaParser(),
  new RawParser(),
])

/** Represents a config token with its value(s). */
export class Token implements ToCacheKey {
  constructor(
    /** Token path in the tokens object. */
    readonly path: Path,
    /** Unparsed value of the token. */
    readonly raw: string,
    /** Token value(s). */
    readonly values: Array<TokenValue>,
  ) {}

  /** Creates a token from a path and raw value. */
  static from(rawPath: Array<string> | Path, raw: string): Token {
    const path = Array.isArray(rawPath) ? new Path(rawPath) : rawPath
    const values = parseValues(raw)

    return new Token(path, raw, values)
  }

  toCacheKey(): string {
    return `${this.path}:${hash(this.raw)}`
  }
}

function parseValues(raw: string): Array<TokenValue> {
  const values: Array<TokenValue> = []

  const ast = parse(raw, {
    context: 'value',
    positions: true,
  })

  walk(ast, (root) => {
    if (root.type === 'Value') {
      for (const node of root.children) {
        // Get the node value from the input string using the node location.
        const start = node.loc?.start.offset ?? 0
        const end = node.loc?.end.offset ?? raw.length
        const input = raw.substring(start, end)

        // Actually parse the value.
        const value = valueParserChain.parse({ input, node })

        if (value) {
          values.push(value)
        }
      }
    }
  })

  return values
}
